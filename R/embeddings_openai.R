...
#' @param model Character scalar; embedding model name, e.g.,
...
get_openai_embeddings <- function(
  ...
  if (any(is.na(texts))) {
    stop("texts must not contain NA values.", call. = FALSE)
  }
  ...
  if (!is.character(api_key) || length(api_key) != 1L || !nzchar(api_key)) {
    stop("api_key must be a single non-empty character string.", call. = FALSE)
  }
...
#'
#' Used by the RAG pipeline and RAGAS-style metric prompts to generate text from
#' a constructed prompt.
#'
#' @param prompt Character scalar; the user/content prompt.
#' @param model Character scalar; chat model name, e.g., `"gpt-4o-mini"`.
#' @param system_message Optional API-level system message. If NULL or empty,
#'   no system message is sent.
#' @param temperature Numeric; sampling temperature.
#' @param max_output_tokens Integer or NULL; maximum tokens to generate. If NULL,
#'   the parameter is omitted from the request.
#' @param seed Optional integer. If provided and supported by the model/provider,
#'   it is sent as `seed` to encourage reproducible outputs.
#' @param api_key OpenAI API key. If NULL, uses the `OPENAI_API_KEY`
#'   environment variable.
#'
...
generate_openai_chat <- function(
  ...
  if (!is.null(system_message)) {
    if (!is.character(system_message) || length(system_message) != 1L) {
      stop("system_message must be NULL or a single character string.", call. = FALSE)
    }
  }

  if (!is.null(max_output_tokens)) {
    if (!is.numeric(max_output_tokens) || length(max_output_tokens) != 1L) {
      stop("max_output_tokens must be NULL or a single numeric value.", call. = FALSE)
    }
    max_output_tokens <- as.integer(max_output_tokens)
    if (is.na(max_output_tokens) || max_output_tokens <= 0L) {
      stop("max_output_tokens must be NULL or a positive integer.", call. = FALSE)
    }
  }
  ...
  if (!is.null(system_message) && nzchar(trimws(system_message))) {
    messages <- c(
      messages,
